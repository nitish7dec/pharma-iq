...
BASE              = "/Users/nitishkaushik/Desktop/pharma_rag"
...
# Base layout — NO legend key here, add per chart
BASE_LAYOUT = dict(
...
You are a dynamic assistant with access to pharmaceutical company data including
annual reports (10-K filings) and clinical trial data from ClinicalTrials.gov.

You must automatically:
- Detect which company the user is asking about from their question
...
Rules:
1. Answer ONLY from provided context. Never use general knowledge.
2. Always cite: (Source: Company | Doc Type | Year | Page X)
3. If context is insufficient: "I don't have sufficient data to answer this."
4. Be precise with all numbers — always include units.
5. For multi-company questions use clear structured headings.
6. Never fabricate data, statistics, or trial results.
7. If company/year not in data, say so clearly.
8. If partial data available, state what you found and suggest a more specific question.
...
def rewrite_query(q):
    ...
    m = c.messages.create(model=CLAUDE_MODEL, max_tokens=60,
        messages=[{"role":"user","content":f"Fix typos and rewrite clearly. Return ONLY the rewritten question.\nQuestion: {q}"}])
...
def query_pipeline(user_question):
    ...
    ar = client.get_collection("annual_reports")
    cr = client.get_collection("clinical_trials")

    a_res = ar.query(query_embeddings=[qvec], n_results=10, include=["documents","metadatas","distances"])
    c_res = cr.query(query_embeddings=[qvec], n_results=4,  include=["documents","metadatas","distances"])

    results = (list(zip(a_res["documents"][0], a_res["metadatas"][0], a_res["distances"][0])) +
               list(zip(c_res["documents"][0], c_res["metadatas"][0], c_res["distances"][0])))

    context = ""
    for i,(doc,meta,_) in enumerate(results):
        context += f"\n--- Source {i+1} ---\nCompany: {meta['company']}\nDoc Type: {meta['doc_type']}\nYear: {meta['year']}\nPage: {meta['page']}\nContent: {doc}\n"

    prompt  = f"Context:\n{context}\n\nQuestion: {clean}\n\nAnswer based only on context. Cite sources clearly."
    claude  = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)
    msg     = claude.messages.create(model=CLAUDE_MODEL, max_tokens=MAX_TOKENS,
    ...
    for _,meta,score in results:
        k = f"{meta['company']}|{meta['doc_type']}|{meta['year']}|{meta['page']}"
        if k not in seen:
            seen.add(k)
...
if st.session_state.view == "dashboard":
    ...
    cols = st.columns(6)
    kpis = [
        ("Pfizer Revenue","$58.5B","FY 2023 · ↑8.8% in 2024","linear-gradient(90deg,#1D4ED8,#60A5FA)","#EFF6FF","#1D4ED8","Pfizer"),
        ("Eli Lilly Revenue","$34.1B","FY 2023 · ↑31.9% by 2025","linear-gradient(90deg,#BE123C,#F87171)","#FFF1F2","#BE123C","Eli Lilly"),
    ...
    with c3:
        ...
        for x0,x1,y0,y1,col,lbl in [
            ...
            fig_mq.add_shape(type="rect",x0=x0,x1=x1,y0=y0,y1=y1,
                fillcolor=col,line=dict(width=0),layer="below")
            fig_mq.add_annotation(x=(x0+x1)/2,y=(y0+y1)/2,text=lbl,
                font=dict(size=9,color="#C8D5E0",family="Plus Jakarta Sans",weight=700),
                showarrow=False)
        # Dividers
        ...
        # Eli Lilly — smaller bubble, correct position
        fig_mq.add_trace(go.Scatter(
            x=[7.1], y=[9.1], mode="markers+text",
            ...
            name="Eli Lilly", showlegend=False,
            hovertemplate="<b>Eli Lilly</b><br>Execution: 7.1/10<br>Vision: 9.1/10<extra></extra>"))
        # Pfizer — smaller bubble
        fig_mq.add_trace(go.Scatter(
            x=[8.2], y=[6.8], mode="markers+text",
            marker=dict(size=28, color=PFE_COLOR, opacity=0.9, line=dict(color="white",width=2)),
            text=["Pfizer"], textposition="middle center",
            textfont=dict(color="white",size=10,family="Plus Jakarta Sans"),
            name="Pfizer", showlegend=False,
            hovertemplate="<b>Pfizer</b><br>Execution: 8.2/10<br>Vision: 6.8/10<extra></extra>"))
        ...
        st.markdown('<div class="insight">💡 <strong>Eli Lilly leads on Vision</strong> — highest R&D intensity (27.1%), GLP-1 dominance, Alzheimer pipeline. <strong>Pfizer leads on Execution</strong> — larger revenue base, global commercial infrastructure.</div></div>', unsafe_allow_html=True)
    ...
    with c4:
        ...
        fig_r.add_trace(go.Scatterpolar(
            r=pfe_s+[pfe_s[0]], theta=dims+[dims[0]],
            fill="toself", name="Pfizer",
            line=dict(color=PFE_COLOR,width=2),
            fillcolor="rgba(29,78,216,0.1)",
            marker=dict(size=5,color=PFE_COLOR)))
        fig_r.add_trace(go.Scatterpolar(
            r=lly_s+[lly_s[0]], theta=dims+[dims[0]],
            fill="toself", name="Eli Lilly",
            line=dict(color=LLY_COLOR,width=2),
            fillcolor="rgba(190,18,60,0.08)",
            marker=dict(size=5,color=LLY_COLOR)))
        # Note: radar uses its own legend placement — don't pass legend in BASE_LAYOUT
        ...
        st.markdown('<div class="insight">💡 Eli Lilly dominates <strong>R&D Intensity & Profit Margin</strong>. Pfizer leads on <strong>Revenue Scale & Therapeutic Diversity</strong>. Closely matched on pipeline depth.</div></div>', unsafe_allow_html=True)
    ...
    with c5:
        ...
        fig_d1.add_annotation(text="<b>803</b><br><span style='font-size:10px'>Trials</span>",
            x=0.5,y=0.5,font=dict(size=13,color="#0F172A"),showarrow=False)
    ...
    with c6:
        ...
        fig_d2 = go.Figure(go.Pie(
            ...
            textinfo="percent+label", textfont=dict(size=9,family="Plus Jakarta Sans"),
            hovertemplate="<b>%{label}</b><br>%{value} trials<extra></extra>"))
        fig_d2.add_annotation(text="<b>775</b><br><span style='font-size:10px'>Trials</span>",
            x=0.5,y=0.5,font=dict(size=13,color="#0F172A"),showarrow=False)
    ...
    with c7:
        ...
        fig_st = go.Figure()
        statuses = ["Completed","Recruiting","Active","Terminated","Withdrawn"]
        pfe_sv   = [500,89,71,118,18]
        lly_sv   = [506,109,85,53,13]
        sc       = ["#22C55E","#1D4ED8","#F59E0B","#EF4444","#94A3B8"]
        for s,pv,lv,c in zip(statuses,pfe_sv,lly_sv,sc):
            fig_st.add_trace(go.Bar(name=s,x=["Pfizer","Eli Lilly"],y=[pv,lv],
                marker_color=c,text=[pv,lv],textposition="inside",
    ...
    # Heatmap
    st.markdown('<div class="chart-card"><div class="chart-title">Therapeutic Area Heatmap</div><div class="chart-sub">Trial count per disease area · Darker = more trials · Reveals strategic focus and competitive positioning</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="insight">💡 <strong>Pfizer dominates Oncology (151) & Infectious Disease (91)</strong> — Seagen acquisition + vaccine heritage. <strong>Eli Lilly leads Diabetes (130), Obesity (73) & Immunology (94)</strong> — directly aligned with Mounjaro, Zepbound & Taltz.</div></div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════
    ...
    <div class="chat-hero">
        ...
        <div>
            ...
            <span class="hero-chip">⚡ RAG Pipeline</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not st.session_state.messages:
        ...
        suggestions = [
            ...
            "Compare R&D expenses of Pfizer and Eli Lilly",
            ...
            "What risks did Pfizer mention in 2024?",
                ...
                if "sources" in msg and msg["sources"]:
                    with st.expander(f"📎 {len(msg['sources'])} sources"):
                        for src in msg["sources"]:
                            css = "sp-p" if src["company"]=="pfizer" else "sp-l"
                            co  = "Pfizer" if src["company"]=="pfizer" else "Eli Lilly"
                            ico = "📄" if src["doc_type"]=="annual" else "🧬"
                            st.markdown(f'<span class="source-pill {css}">{ico} {co} · {src["doc_type"].title()} · {src["year"]} · p.{src["page"]}</span>', unsafe_allow_html=True)
                if "tokens" in msg:
    ...
    if st.session_state.messages:
        if st.button("🗑️ Clear chat", key="clr"):
            st.session_state.messages=[]; st.session_state.total_tokens=0
            st.session_state.total_cost=0.0; st.session_state.query_count=0; st.rerun()
...
# ════════════════════════════════════════════════
    ...
    # Waterfall — fixed x-axis
    st.markdown('<div class="chart-card"><div class="chart-title">Pfizer Revenue Journey — The COVID Cliff Story</div><div class="chart-sub">USD millions · FY2021 → FY2024 · Rise and fall of COVID products</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="insight">💡 Pfizer surged from <strong>$81B (FY2021) to $100B (FY2022)</strong> on COVID vaccine & Paxlovid — then crashed to <strong>$58.5B in FY2023</strong>. Recovering to <strong>$63.6B in FY2024</strong>. One of pharma history\'s most dramatic revenue swings.</div></div>', unsafe_allow_html=True)

    cv1, cv2 = st.columns(2)

    with cv1:
        st.markdown('<div class="chart-card"><div class="chart-title">R&D Efficiency — Bubble Analysis</div><div class="chart-sub">R&D spend vs Net Income · Bubble size = Revenue · Who converts R&D to profit?</div>', unsafe_allow_html=True)
        fig_sc = go.Figure()
        companies  = ["Pfizer FY23","Pfizer FY24","Eli Lilly FY23","Eli Lilly FY25"]
        rd_vals    = [10679,10953,9234,12155]
        ni_vals    = [2233,8030,5240,10591]
        rev_vals   = [58496,63625,34124,45042]
        clrs       = [PFE_COLOR,PFE_COLOR,LLY_COLOR,LLY_COLOR]
        for i in range(4):
            fig_sc.add_trace(go.Scatter(
                x=[rd_vals[i]], y=[ni_vals[i]], mode="markers+text",
                marker=dict(size=rev_vals[i]/1200, color=clrs[i], opacity=0.75, line=dict(color="white",width=2)),
                text=[companies[i]], textposition="top center",
                textfont=dict(size=10,family="Plus Jakarta Sans"),
                showlegend=False,
                hovertemplate=f"<b>{companies[i]}</b><br>R&D: ${rd_vals[i]:,}M<br>Net: ${ni_vals[i]:,}M<br>Rev: ${rev_vals[i]:,}M<extra></extra>"))
        ...
        st.markdown('<div class="insight">💡 <strong>Eli Lilly converts R&D into profit far more efficiently.</strong> Similar R&D spend as Pfizer but 2.3x more net income. Pfizer FY2024 shows strong recovery.</div></div>', unsafe_allow_html=True)

    with cv2:
        st.markdown('<div class="chart-card"><div class="chart-title">Phase 3 Pipeline — Butterfly Chart</div><div class="chart-sub">Head-to-head Phase 3 trial comparison by therapeutic area</div>', unsafe_allow_html=True)
        ...
        fig_p3.add_trace(go.Bar(name="Pfizer", y=areas_p3, x=pfe_p3, orientation="h",
            marker_color=PFE_COLOR, opacity=0.85,
            text=pfe_p3, textposition="outside",
        ...
        fig_p3.add_trace(go.Bar(name="Eli Lilly", y=areas_p3, x=[-v for v in lly_p3], orientation="h",
            marker_color=LLY_COLOR, opacity=0.85,
            text=lly_p3, textposition="outside",
        ...
        fig_p3.update_layout(**BASE_LAYOUT, height=280, barmode="relative",
            ...
            xaxis=dict(showgrid=False,tickvals=[-60,-40,-20,0,20,40,60],
                ...
                tickfont=dict(size=10), title="← Eli Lilly  |  Pfizer →",
                title_font=dict(size=10,color="#64748B")),
            yaxis=dict(showgrid=False,tickfont=dict(size=10)))
        st.plotly_chart(fig_p3, use_container_width=True, config=PLOTLY_CFG)
        st.markdown('<div class="insight">💡 <strong>Eli Lilly\'s Phase 3 pipeline dominates Diabetes, Obesity & Immunology</strong> — matching their marketed blockbusters. Pfizer\'s Phase 3 strength is Oncology & Infectious Disease.</div></div>', unsafe_allow_html=True)
    ...
    fig_m.add_trace(go.Scatter(x=["FY2023","FY2024"], y=[70.0,70.8], mode="lines+markers+text",
        name="Pfizer Gross Margin", line=dict(color=PFE_COLOR,width=2.5),
        marker=dict(size=8,color=PFE_COLOR),
        text=["70.0%","70.8%"], textposition="top center",
        textfont=dict(size=10,family="JetBrains Mono",color=PFE_COLOR)))
    fig_m.add_trace(go.Scatter(x=["FY2023","FY2024"], y=[3.8,12.6], mode="lines+markers+text",
        name="Pfizer Net Margin", line=dict(color=PFE_COLOR,width=2,dash="dot"),
        marker=dict(size=8,color=PFE_COLOR),
        text=["3.8%","12.6%"], textposition="bottom center",
        textfont=dict(size=10,family="JetBrains Mono",color=PFE_COLOR)))
    fig_m.add_trace(go.Scatter(x=["FY2023","FY2025"], y=[83.5,86.4], mode="lines+markers+text",
        name="Eli Lilly Gross Margin", line=dict(color=LLY_COLOR,width=2.5),
        marker=dict(size=8,color=LLY_COLOR),
        text=["83.5%","86.4%"], textposition="top center",
        textfont=dict(size=10,family="JetBrains Mono",color=LLY_COLOR)))
    fig_m.add_trace(go.Scatter(x=["FY2023","FY2025"], y=[15.4,23.5], mode="lines+markers+text",
        name="Eli Lilly Net Margin", line=dict(color=LLY_COLOR,width=2,dash="dot"),
        marker=dict(size=8,color=LLY_COLOR),
        text=["15.4%","23.5%"], textposition="bottom center",
        textfont=dict(size=10,family="JetBrains Mono",color=LLY_COLOR)))
    ...
    st.markdown('<div class="insight">💡 <strong>Eli Lilly gross margin (83.5-86.4%) far outperforms Pfizer (70%)</strong> — superior pricing power. Eli Lilly net margin improving 15.4%→23.5%. Pfizer recovering strongly 3.8%→12.6%.</div></div>', unsafe_allow_html=True)
