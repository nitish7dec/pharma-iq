...
.sp-o { background:#F0FDF4; color:#15803D; border:1px solid #BBF7D0; }
...
def query_pipeline(user_question):
    ...
    # 10 from annual reports
    a_res = index.query(
        vector           = qvec,
        top_k            = 10,
        include_metadata = True,
        filter           = {"doc_type": {"$eq": "annual"}}
    )

    # 4 from clinical trials
    c_res = index.query(
        vector           = qvec,
        top_k            = 4,
        include_metadata = True,
        filter           = {"doc_type": {"$eq": "clinical"}}
    )

    # Combine results
    results = []
    for match in a_res["matches"] + c_res["matches"]:
        results.append((
...
if st.session_state.view == "dashboard":
    ...
    with c4:
        ...
        fig_r = go.Figure()
        fig_r.add_trace(go.Scatterpolar(r=pfe_s+[pfe_s[0]],theta=dims+[dims[0]],fill="toself",name="Pfizer",
            line=dict(color=PFE_COLOR,width=2),fillcolor="rgba(29,78,216,0.1)",marker=dict(size=5,color=PFE_COLOR)))
        fig_r.add_trace(go.Scatterpolar(r=lly_s+[lly_s[0]],theta=dims+[dims[0]],fill="toself",name="Eli Lilly",
            line=dict(color=LLY_COLOR,width=2),fillcolor="rgba(190,18,60,0.08)",marker=dict(size=5,color=LLY_COLOR)))
    ...
    st.markdown('<div class="insight">💡 <strong>Pfizer dominates Oncology (151) & Infectious Disease (91)</strong> — Seagen acquisition + vaccine heritage. <strong>Eli Lilly leads Diabetes (130), Obesity (73) & Immunology (94)</strong> — aligned with Mounjaro, Zepbound & Taltz.</div></div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════
    ...
    <div class="chat-hero">
        ...
        <p>Ask anything about the companies in our knowledge base — financials,
           clinical trials, R&D strategy, risk factors and more. No filters needed.</p>
        <div>
                ...
                if "sources" in msg and msg["sources"]:
                    with st.expander(f"📎 {len(msg['sources'])} sources"):
                        for src in msg["sources"]:
                            ...
                            if company == "pfizer":
                                css, co = "sp-p", "Pfizer"
                            elif company == "eli_lilly":
                                css, co = "sp-l", "Eli Lilly"
                            else:
                                css, co = "sp-o", company.title()
...
# ════════════════════════════════════════════════
    ...
    with cv1:
        ...
        rd_v=[10679,10953,9234,12155]; ni_v=[2233,8030,5240,10591]
        rv_v=[58496,63625,34124,45042]; clrs=[PFE_COLOR,PFE_COLOR,LLY_COLOR,LLY_COLOR]
        for i in range(4):
            fig_sc.add_trace(go.Scatter(x=[rd_v[i]],y=[ni_v[i]],mode="markers+text",
                marker=dict(size=rv_v[i]/1200,color=clrs[i],opacity=0.75,line=dict(color="white",width=2)),
                text=[companies[i]],textposition="top center",
                textfont=dict(size=10,family="Plus Jakarta Sans"),showlegend=False,
                hovertemplate=f"<b>{companies[i]}</b><br>R&D: ${rd_v[i]:,}M<br>Net: ${ni_v[i]:,}M<extra></extra>"))
    ...
    with cv2:
        ...
        st.markdown('<div class="insight">💡 <strong>Eli Lilly Phase 3 dominates Diabetes, Obesity & Immunology</strong> — matching their blockbusters. Pfizer Phase 3 strength is Oncology & Infectious Disease.</div></div>', unsafe_allow_html=True)
